...
const dashboardData = [];
for (const [scenarioName, priceData] of Object.entries(ethPriceScenarios)) {
  const scenarioData = createScenarioSheet(priceData);
  const lastMonth = scenarioData[scenarioData.length - 1];

  dashboardData.push({
    Scenario: scenarioName,
    "Final ETH Price ($)": lastMonth["ETH Price at Deposit ($/ETH)"],
    "Total Deposited ($)": (monthlyInvestment * months).toFixed(2),
    "Total ETH Held (With Interest)": lastMonth["Total ETH Held With Interest (ETH)"],
    "Final ETH Value ($)": lastMonth["Total ETH Value (USD)"],
    "Total Borrowed ($)": lastMonth["Total Borrowed with Interest ($)"],
    "Final LP Value ($)": lastMonth["LP Value After IL ($)"],
    "Total Gas Fees Paid ($)": (totalGasFeePerMonth * months).toFixed(2),
    "Final Net Capital ($)": lastMonth["Net Capital ($)"],
    "Profit ($)": lastMonth["Profit ($)"],
    "LTV at Final Month (%)": lastMonth["LTV (%)"],
    "Liquidated?": lastMonth["Liquidated?"]
  });
}
...
// === Save File ===
const filename = 'ETH_LP_Strategy_with_Dashboard.xlsx';
